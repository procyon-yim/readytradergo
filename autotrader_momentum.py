    ...
    def on_order_book_update_message(self, instrument: int, sequence_number: int, ask_prices: List[int],
        ...
        if instrument == Instrument.ETF:
            ...
            if n>m:
                ...
                SMA = TP_list[n-1]
                UB = SMA + np.std(TP_list)
                MB = TP_list[n-1]
                LB = SMA - np.std(TP_list)
            ...
            if self.bid_id == 0 and new_bid_price != 0 and self.position < POSITION_LIMIT and new_bid_price < LB:
                self.bid_id = next(self.order_ids)
                self.bid_price = new_bid_price
                self.send_insert_order(self.bid_id, Side.SELL, new_ask_price, LOT_SIZE, Lifespan.GOOD_FOR_DAY)
                self.bids.add(self.bid_id)

            if self.ask_id == 0 and new_ask_price != 0 and self.position > -POSITION_LIMIT and new_ask_price > UB:
