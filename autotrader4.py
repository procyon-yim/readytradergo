...
class AutoTrader(BaseAutoTrader):
    ...
    def on_hedge_filled_message(self, client_order_id: int, price: int, volume: int) -> None:
        ...
        self.logger.info("received hedge filled for order %d with average price %d and volume %d etf volume %f", client_order_id,
        ...
        if instrument == Instrument.FUTURE:
            ...
            if self.bid_id == 0 and new_bid_price != 0:
                if abs(self.position) + LOT_SIZE > POSITION_LIMIT:
                    TEMP_LOT_SIZE = int(abs(POSITION_LIMIT - self.position))
                    ...
                    self.logger.info("received order book for BUY instrument %d with sequence number %d current "
                                     ...
                                     instrument, sequence_number, self.position, TEMP_LOT_SIZE, self.bid_id)
                ...
                # instrument, sequence_number, self.etf_volume, TEMP_LOT_SIZE)
                    ...
                    self.logger.info("received order book for BUY instrument %d with sequence number %d current "
                                     "volume %f, temp lot size %f with order id %f",
                                     instrument, sequence_number, self.position, LOT_SIZE, self.bid_id)
            ...
            if self.ask_id == 0 and new_ask_price != 0:
                if abs(self.position) + LOT_SIZE > POSITION_LIMIT:
                    ...
                    self.logger.info("received order book for SELL instrument %d with sequence number %d current "
                                     "volume %f, temp lot size %f with order id %f",
                                     instrument, sequence_number, self.position, TEMP_LOT_SIZE, self.ask_id)
