...
 
    ...
    def on_order_book_update_message(self, instrument: int, sequence_number: int, ask_prices: List[int],
        ...
        if instrument == Instrument.FUTURE:
            ...
            if self.bid_id == 0 and new_bid_price != 0 and  LLB < new_bid_price < LB:
            ...
            if self.ask_id == 0 and new_ask_price != 0 and UUB > new_ask_price > UB:
                if abs(self.etf_volume) + LOT_SIZE > POSITION_LIMIT:
                    TEMP_LOT_SIZE = int((POSITION_LIMIT + self.etf_volume))
            ...
            if new_bid_price < LLB and self.ask_id == 0 and new_ask_price != 0:
                if abs(self.etf_volume) + LOT_SIZE > POSITION_LIMIT:
                    TEMP_LOT_SIZE = int((POSITION_LIMIT + self.etf_volume))
                    self.ask_id = next(self.order_ids)
                    self.ask_price = new_ask_price
            ...
            if new_ask_price > UUB and self.bid_id == 0 and new_bid_price != 0:
                if abs(self.etf_volume) + LOT_SIZE > POSITION_LIMIT:
                    TEMP_LOT_SIZE = int((POSITION_LIMIT - self.etf_volume))
                    self.bid_id = next(self.order_ids)
                    self.bid_price = new_bid_price
